$(function() {
	...
	$(window).scroll(function(){
		if($('[id^="calendar_popup"]').length > 0)
		{
			$('[id^="calendar_popup"]').css({'top': ($('[name="bdate"]').offset().top+$('[name="bdate"]').outerHeight()) + 'px'});
		}
	});
	...
	$('#join_form').submit(function(e){
		...
		var form = $(this);
		var err = false;
		$('fieldset.active [required="required"]', form).each(function(index, element) {
            if(!$(this).val())
			{
				err = true;
				$(this).parent().addClass('hasError');
